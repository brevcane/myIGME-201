...
namespace BTree
{
    public partial class BTreeForm : Form
    {
        ...
        private void Exercise2__Click(object sender, EventArgs e)
        {
            ...
            {
                int value = (i + 1) * (51 / 8);
                node = new BTree(value, root, false);
            }
        ...
        private void Exercise3__Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < 15; ++i)
            {
                node = new BTree(alphabet[random.Next(0,26)], root);
        ...
        private void Exercise6__Click(object sender, EventArgs e)
        {
            ...
            // It appears that in Classes.cs the overloads already support the person object and use age to compare the Person class.

            root = new BTree(new Person("BrevinHorton", 19), null);
            node = new BTree(new Person("TrentHorton", 19), root); 
            node = new BTree(new Person("TylerHorton", 19), node);
            node = new BTree(new Person("KaylieHorton", 19), node);
            node = new BTree(new Person("MadisonHorton", 16), node);
            node = new BTree(new Person("TroyHorton", 52), node);
            node = new BTree(new Person("RachaelHorton", 43), node);
            node = new BTree(new Person("MorganHorton", 73), node);
            node = new BTree(new Person("JoyceHorton", 69), node);
            node = new BTree(new Person("RickHorton", 46), node);
            node = new BTree(new Person("JeffCouperous", 45), node);
            node = new BTree(new Person("RickyCouperous", 51), node);
            node = new BTree(new Person("TiaCouperous", 44), node);
            node = new BTree(new Person("BarberaScharett", 71), node);
            node = new BTree(new Person("DougScharett", 75), node);
        ...
        private void Exercise7__Click(object sender, EventArgs e)
        {
            ...
            // Your code here
            int[] optimalAges = { 16, 19, 19, 19, 43, 44, 45, 46, 51, 52, 69, 71, 73, 75 };

            // Prime the tree with nodes containing Person objects
            for (int i = 0; i < optimalAges.Length; i++)
            {
                node = new BTree(new Person("OptimalPerson" + i, optimalAges[i]), root, false);
                root = node;
            }
